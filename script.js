/* function rotate({x1, y1, z1, x2, y2, z2, angle}) {
    let d = Math.sqrt(x2**2 + y2**2 + z2**2);
    [x2, y2, z2] = [x2/d, y2/d, z2/d];
    let q = {
        r: Math.cos(angle/2),
        i: Math.sin(angle/2)*x2,
        j: Math.sin(angle/2)*y2,
        k: Math.sin(angle/2)*z2,
    }
    let newX = x1*(1-2*q.j**2-2*q.k**2) + y1*(q.i*q.j-q.k*q.r)*2 + z1*(q.i*q.k+q.j*q.r)*2;
    let newY = x1*(q.i*q.j+q.k*q.r)*2 + y1*(1-2*q.i**2-2*q.k**2) + z1*(q.j*q.k-q.i*q.r)*2;
    let newZ = x1*(q.i*q.k-q.j*q.r)*2 + y1*(q.j*q.k+q.i*q.r)*2 + z1*(1-2*q.i**2-2*q.j**2);
...
let glob = (function(){
    let fov = Math.PI/2;
    let distance = canvas.width/3;
    let sensitivity = 1/300;
    let pressedKeys = {};
    let moveSensitivity = 1/10;
...
let environment = (function(){
    ...
    function newCube(center, size) {
        let cube = {
            points: [
                {x:center.x-size/2,y:center.y-size/2,z:center.z-size/2},
                {x:center.x-size/2,y:center.y-size/2,z:center.z+size/2},
                {x:center.x-size/2,y:center.y+size/2,z:center.z-size/2},
                {x:center.x-size/2,y:center.y+size/2,z:center.z+size/2},
                {x:center.x+size/2,y:center.y-size/2,z:center.z-size/2},
    ...
    function calcObjPos(obj) {
        obj.renderedPoints = obj.points.map(function(el){
            return {
                x: el.x/el.z/Math.tan(glob.fov/2)*glob.distance,
                y: -el.y/el.z/Math.tan(glob.fov/2)*glob.distance,
                z: el.z 
            }
        });
        return obj;
    ...
    for (let i=-size; i<=size; i++) {
        for (let j=-size; j<=size; j++) {
            for (let k=-size; k<=size; k++) {
                ...
                newCube({x: i*15, y: j*15, z: k*15}, 5);
            }   
        }
    }
...
let rctx = (function(){
    function renderObj(obj) {
        ...
        obj.lines.forEach(function(el){
            if (obj.renderedPoints[el.from].z>0 && obj.renderedPoints[el.to].z>0) {
                ctx.beginPath();
                ctx.moveTo(obj.renderedPoints[el.from].x+canvas.width/2, obj.renderedPoints[el.from].y+canvas.height/2);
                ctx.lineTo(obj.renderedPoints[el.to].x+canvas.width/2, obj.renderedPoints[el.to].y+canvas.height/2);
                ctx.stroke();
            }
...
document.addEventListener('keydown', function(e){
    glob.pressedKeys[e.key] = true;
})

document.addEventListener('keyup', function(e){
    glob.pressedKeys[e.key] = false;
})

document.addEventListener('wheel', function(e){
    glob.moveSensitivity -= e.deltaY/5000;
    glob.moveSensitivity = Math.max(Math.min(3/4, glob.moveSensitivity), 1/100);
})